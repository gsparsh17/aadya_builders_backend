...
class UserController {
  ...
  async updatePreferences(req, res, next) {
    try {
      ...
      // ✅ Transform preferredLocations from strings to objects
      if (preferences.preferredLocations && Array.isArray(preferences.preferredLocations)) {
        ...
        if (preferences.preferredLocations.length > 0 &&
          typeof preferences.preferredLocations[0] === 'string') {
          // Transform to object format: { city: string, locality: [] }
          preferences.preferredLocations = preferences.preferredLocations.map(city => ({
            city: city,
            locality: []
          }));
        }
      ...
      // ✅ Transform propertyTypes from strings to objects
      if (preferences.propertyTypes && Array.isArray(preferences.propertyTypes)) {
        // Check if it's an array of strings
        if (preferences.propertyTypes.length > 0 &&
          typeof preferences.propertyTypes[0] === 'string') {
          // Transform to object format: { type: string }
          preferences.propertyTypes = preferences.propertyTypes.map(type => ({
            type: type
          }));
        }
      }

      // ✅ Transform preferredConfigurations from strings to objects (if needed)
      if (preferences.preferredConfigurations && Array.isArray(preferences.preferredConfigurations)) {
        if (preferences.preferredConfigurations.length > 0 &&
          typeof preferences.preferredConfigurations[0] === 'string') {
          preferences.preferredConfigurations = preferences.preferredConfigurations.map(config => ({
            type: config
          }));
        }
      }

      // ✅ Transform budgetRange - ensure numbers are valid
      ...
      // ✅ Keep notificationPreferences as is (it's already an object)
      ...
      // ✅ Transform response back to simple format for frontend
      const responsePreferences = {
        preferredLocations: updatedPreferences.preferredLocations?.map(loc => loc.city) || [],
        propertyTypes: updatedPreferences.propertyTypes?.map(type => type.type) || [],
        budgetRange: updatedPreferences.budgetRange || { min: 0, max: 0 },
        preferredConfigurations: updatedPreferences.preferredConfigurations?.map(config => config.type) || [],
        furnishingPreference: updatedPreferences.furnishingPreference,
      ...
      return successResponse(res, responsePreferences, 'Preferences updated successfully');
